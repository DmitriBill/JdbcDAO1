...
public class Main {
    public static void main(String[] args) throws SQLException {
        try (Connection conn = ConnectionFactory.getConnection())
        {
            ...
            dao.add(c);
            for (int i = 0; i < 10; i++) {
                Client client = new Client("Client" + i, 20 + i);
                dao.add(client);
            }
            // int id = c.getId();
            ...
            List<Client> num1 = dao.getAll(Client.class, "name", "age");
            for (Client cli : num1)  System.out.println(cli);

            List<Client> num2 = dao.getAll(Client.class, "age");
            for (Client cli : num2)  System.out.println(cli);

            List<Client> num3 = dao.getAll(Client.class, "name");
            for (Client cli : num3)  System.out.println(cli);
