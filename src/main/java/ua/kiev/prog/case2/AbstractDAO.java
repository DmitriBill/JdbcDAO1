...
public abstract class AbstractDAO<T> {
    ...
    public List<T> getAll (Class<T> cls, String prm1, String prm2) {
        List<T> cl = new ArrayList<>();
        try {
            try (Statement st = conn.createStatement()) {
                try (ResultSet rs = st.executeQuery("select " + prm1 + ", " + prm2 + " from " + table)) {
                    this.objectGen(cls,rs,cl);
                }
            }
        } catch (Exception ex) {
            throw new RuntimeException(ex);
    ...
    public List<T> getAll (Class<T> cls, String prm) {
        List<T> cl = new ArrayList<>();
        try (Statement st = conn.createStatement()) {
            try (ResultSet rs = st.executeQuery("select "+prm+" from "+ table)) {
                this.objectGen(cls,rs,cl);
            }
        } catch (Exception ex) {
            throw new RuntimeException(ex);
    ...
    private void objectGen (Class<T> cls, ResultSet rs, List<T> cl) throws SQLException, InstantiationException, IllegalAccessException, NoSuchFieldException {
        ...
        while (rs.next()) {
            ...
            for (int i = 1; i <= md.getColumnCount(); i++) {
                ...
                Field field = cls.getDeclaredField(cn);
